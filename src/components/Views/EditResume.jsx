...
import Guage from '../Blocks/Guage';
import JobForm from '../Blocks/JobForm';
...
const EditResume = () => {
	...
	const getJob = async () => {
		try {
			const reqPromise = await fetch(`http://localhost:5173/api${path}`, {
				method: 'GET',
			})
				.then((res) => res.json())
				.then((res) => {
			...
			<MainBody sx={{ paddingTop: '200px' }}>
				<Stack spacing={2}>
					...
					{path && job && job.company_name && job.requirements && (
						...
						<JobForm
							createJob={false}
							saveJobUrl={`http://localhost:5173/api${path}`}
